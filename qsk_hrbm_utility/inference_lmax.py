...
def calc_exact_ee(N, seed_qsk, alpha, path=None):
    ...
    _, ground_state_vec = generate_state_sk(N=N, seed=seed_qsk, path=path)
...
def load_rbm_vstate(N, hi, chkpt_path, alpha):
    ...
    if not os.path.exists(rbm_file):
        rbm_file = os.path.join(chkpt_path, "rbm_alpha1_ML_final_params.mpack")
...
def load_hrbm_vstate(N, hi, chkpt_path, alpha, Lmax):
    ...
    if not os.path.exists(hrbm_file):
        hrbm_file = os.path.join(
            #chkpt_path, f"hrbm_alpha={alpha}_Lmax={Lmax}_ML_final_params.mpack" #Uncheck this later
            chkpt_path, f"hrbm_alpha{alpha}_Lmax{Lmax}_ML_final_params.mpack"
            #This is a peculiarity of kaggle naming without'=''
        )
