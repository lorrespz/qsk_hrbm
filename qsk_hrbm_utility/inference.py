...
def define_model_load_vstate(N, hi, seed_qsk, alpha, seed_nqs, path):
...
def evaluate_and_export_energies(
    N, alpha, seed_qsk_list, seed_nqs_list, path, save_dir=None
):
    ...
    for seed_qsk in seed_qsk_list:
        ...
        for seed_nqs in seed_nqs_list:
            ...
            rbm_vstate, hrbm_vstate = define_model_load_vstate(
                N, hi, seed_qsk=seed_qsk, alpha=alpha, seed_nqs=seed_nqs, path
            )
...
#========================================================================================
def entanglement_extraction(N, alpha, seed_qsk_list, seed_nqs_list, path, save_dir=None):
    if save_dir is None:
        save_dir = f"qsk_N={N}"
    os.makedirs(save_dir, exist_ok=True)
...
def plot_qsk_seeds_separately(
    N, alpha, seed_qsk_list, seed_nqs_list, path, save_dir=f"qsk_N=20"
):
    ...
    for idx, seed_qsk in enumerate(seed_qsk_list):
        ...
        for seed_nqs in seed_nqs_list:
            rbm_vstate, hrbm_vstate = define_model_load_vstate(N, hi, seed_qsk=seed_qsk, 
                alpha=alpha, seed_nqs=seed_nqs, path)
